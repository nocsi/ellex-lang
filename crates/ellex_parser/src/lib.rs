...
fn parse_statement(pair: pest::iterators::Pair<Rule>) -> Result<Option<Statement>, pest::error::Error<Rule>> {
    match pair.as_rule() {
        Rule::statement => {
            for inner_pair in pair.into_inner() {
...
fn parse_single_statement(pair: pest::iterators::Pair<Rule>) -> Result<Statement, pest::error::Error<Rule>> {
    match pair.as_rule() {
        Rule::tell_stmt => {
        ...
        _ => {
            ...
            Ok(Statement::Tell(EllexValue::String(format!("Unimplemented: {}", pair.as_str()))))
        }
    }
}

fn parse_expression(pair: pest::iterators::Pair<Rule>) -> Result<EllexValue, pest::error::Error<Rule>> {
    match pair.as_rule() {
        Rule::expression => {
        ...
        Rule::ident => {
            Ok(EllexValue::String(pair.as_str().to_string()))
        }
