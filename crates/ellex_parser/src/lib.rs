...
fn parse_single_statement(
    ...
    match pair.as_rule() {
        ...
        Rule::assignment => {
            ...
            Ok(Statement::Assignment(var_name, value))
        }
        Rule::service_assignment => {
            let mut inner = pair.into_inner();
            let var_name = inner.next().unwrap().as_str().to_string();
            let service_block = inner.next().unwrap();
            // For now, treat service assignments as special assignments
            let service_name = format!("service_{}", var_name);
            Ok(Statement::Assignment(var_name, EllexValue::String(service_name)))
        }
        Rule::repeat_stmt => {
            let mut inner = pair.into_inner();
